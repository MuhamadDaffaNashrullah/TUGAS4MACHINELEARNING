...
def try_load_assets():
    ...
    try:
        # Optimasi: hanya import TensorFlow ketika diperlukan
        ...
        # Optimasi: disable eager execution untuk performa
        ...
        # Optimasi: clear memory setelah loading model
        gc.collect()
...
def predict():
    ...
    try:
        ...
        # Optimasi: disable eager execution untuk prediksi
        import tensorflow as tf
        with tf.device('/CPU:0'):  # Force CPU untuk Railway
            y_scaled = model.predict(x, verbose=0)
        
        y = scaler.inverse_transform(y_scaled.reshape(-1, 1)).ravel()[0]
        
        # Optimasi: clear memory setelah prediksi
        del scaled, x, y_scaled
        gc.collect()
